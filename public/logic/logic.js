$(document).ready(function() {
    ...
    function displayPanel(story) {
        let panel = $(
            [
                '<div class="panel panel-default"data-id-"${story._id}">',
                '<div class="panel-heading">',
                '<h3 class=panel-title">${story.headline}</h3>',
                '</div>',
                '<div class="panel-body">${story.summary}</div>',
                '<div class="panel-footer clearfix">',
                'a class="btn btn-success pull-right archiveStory">Keep It!</a>',
                '<a href="${story.url}" target="_blank" class="btn button-default pull-right">Read It!</a>',
                '</div>',
                '</div>'
            ].join("")
        );
